...
/*
 * Replace "search" with "replace" at the forst occurance after "n" in "subject"
 */
...
function countfinds(search,subject){
	...
	*/
	/*
	while(go > 0){
		//alert("Looking for "+ search +" in "+subject + " starting at " + index);
		go = subject.indexOf(search, index);
		if (go > 0){
			// We found a match
			//alert("We found a match at ["+go+"]");
			ret++;
			index = go+1;
		}
	}
	*/
	/*
	var tmp = subject.match(RegExp(search,'g'));
	if (tmp != null) ret = tmp.length;
*/
	var n = 0;
	var count = 0;
	var index = 0;
	while (n >= 0){
		...
		if (n != -1) count++;
		//console.log(subject.indexOf(search, index));
...
//		var noun,verb,determiner,adjective,adverb,conjunction,modal;
		...
		for(var i=0;i<conjunctions;i++){
			...
			//console.log("After conjunction: "+sentence);							
		}
